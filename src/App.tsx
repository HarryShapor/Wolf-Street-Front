...
function TradingLineChartBackground() {
  const width = 1920, height = 820, points = 320;
  ...
  function generateLine() {
    ...
    for (let i = 1; i < points - 1; i++) {
      ...
      // Волатильность
      y = target + (Math.random() - 0.5) * 80 * (1 - t) + (Math.random() - 0.5) * 40;
  ...
  const polyline = line.slice(0, progress).map((y, i) => `${i * step},${y}`).join(' ');
  ...
  return (
    <div className="trading-bg" style={{zIndex:0}}>
      <svg
        ...
        style={{ position: 'absolute', left: 0, top: 0, width: '100vw', height, opacity: 0.18, pointerEvents: 'none' }}
      >
        <defs>
          <linearGradient id="line-gradient" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={chartColors.up} stopOpacity="0.8" />
            <stop offset="100%" stopColor={chartColors.down} stopOpacity="0.2" />
          </linearGradient>
        </defs>
        <polyline
          points={polyline}
    ...
    <div className="min-h-screen bg-light-bg dark:bg-dark-bg">
      ...
      <Router>
        ...
        <Routes>
          ...
          <Route path="/portfolio" element={<PortfolioPage theme={theme} setTheme={setTheme} NAV={NAV} />} />
