...
import type { IChartApi, CandlestickSeriesOptions, UTCTimestamp, Time } from 'lightweight-charts';
import { useTheme } from '../../context/ThemeContext';
import { FaChevronLeft, FaChevronRight, FaPlus, FaMinus, FaUndo } from 'react-icons/fa';
...
const CandlestickChart: React.FC<CandlestickChartProps> = ({ data }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (series) {
      ...
      const candle = Array.isArray(series.data) ? series.data.find((c: any) => c.time === time) : null;
      if (candle) {
        setCursor('pointer');
      } else {
    ...
    const handleCrosshairMoveLabels = (param: any) => {
      ...
      if (param.time) {
        ...
        if (typeof param.time === 'number') {
          ...
          const localTime = d.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
