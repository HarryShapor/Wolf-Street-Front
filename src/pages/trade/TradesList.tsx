...
const TradesList: React.FC = () => {
  ...
  const instrumentId = instruments.length > 0 ? instruments[0].instrumentId : null;
  ...
  useEffect(() => {
    ...
    setErrorMarket(null);
    fetch(`${API_HOST}/market-data-service/api/v1/trades/${instrumentId}?limit=${VISIBLE_ROWS}`)
      .then(async res => {
      ...
      .then(data => {
        ...
        const trades = Array.isArray(data)
          ? data.map((t: any) => ({
              ...
              time: new Date(t.timestamp || t.time || t.createdAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
            }))
          : [];
        setMarketTrades(trades);
      })
      .catch(e => setErrorMarket(e.message || 'Ошибка загрузки рыночных сделок'))
  ...
  useEffect(() => {
    ...
    setLoadingUser(true);
    setErrorUser(null);
    fetch(`${API_HOST}/order-service/api/v1/order/closed?portfolioId=${portfolioId}`, {
      headers: {
      ...
      .then(data => {
        // Преобразуем в Trade[]
        const trades = Array.isArray(data)
          ? data.slice(-VISIBLE_ROWS).reverse().map((o: any) => ({
              price: o.lotPrice,
              amount: o.count,
              side: (o.side?.toLowerCase() === 'buy' ? 'buy' : 'sell') as 'buy' | 'sell',
              time: new Date(o.closedAt || o.updatedAt || o.createdAt).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
            }))
          : [];
        setUserTrades(trades);
      })
      .catch(e => setErrorUser(e.message || 'Ошибка загрузки завершённых ордеров'))
  ...
  const data = tab === 'market' ? marketTrades.slice(0, VISIBLE_ROWS) : userTrades.slice(0, VISIBLE_ROWS);

  return (
    <div className="relative w-full h-[450px] bg-white/30 dark:bg-dark-card/40 backdrop-blur-md border border-light-border/40 dark:border-dark-border/40 rounded-2xl shadow-2xl transition-all duration-300 hover:shadow-[0_0_32px_0_theme('colors.light-accent')] dark:hover:shadow-[0_0_32px_0_#81c784] hover:scale-[1.03] animate-fadein flex flex-col">
      {/* Вкладки */}
      <div className="flex items-center px-4 pt-2 pb-1 border-b border-light-border/40 dark:border-dark-border/40 bg-light-bg/80 dark:bg-dark-bg/80 rounded-t-2xl gap-2">
        {TABS.map(t => (
          <button
            key={t.key}
            className={`text-xs font-bold tracking-tight px-2 py-1 rounded transition-colors duration-150
              ${tab === t.key ? 'bg-light-accent/20 dark:bg-dark-accent/20 text-light-accent dark:text-dark-accent' : 'text-light-fg-secondary dark:text-dark-brown hover:bg-light-accent/10 dark:hover:bg-dark-accent/10'}`}
      ...
      >
        {tab === 'market' && loadingMarket ? (
          <div className="flex items-center justify-center h-full text-xs text-light-fg-secondary dark:text-dark-brown">Загрузка...</div>
        ) : tab === 'market' && errorMarket ? (
          <div className="flex items-center justify-center h-full text-xs text-red-500 dark:text-red-400">{errorMarket}</div>
        ) : tab === 'user' && loadingUser ? (
          <div className="flex items-center justify-center h-full text-xs text-light-fg-secondary dark:text-dark-brown">Загрузка...</div>
        ) : tab === 'user' && errorUser ? (
          <div className="flex items-center justify-center h-full text-xs text-red-500 dark:text-red-400">{errorUser}</div>
        ) : data.length === 0 ? (
          <div className="flex items-center justify-center h-full text-xs text-light-fg-secondary dark:text-dark-brown">Нет данных</div>
        ) : (
          data.map((t, i) => (
            <div
              key={i}
              ...
              className={
                `grid grid-cols-3 gap-0 px-4 py-0 text-xs leading-[1.1] items-center transition-colors duration-100
                hover:bg-light-accent/10 dark:hover:bg-dark-accent/10 cursor-pointer tracking-tight`
              }
            >
              <span className={`text-left font-bold ${t.side === 'buy' ? 'text-light-success dark:text-dark-accent' : 'text-red-500 dark:text-red-400'}`}>{t.price?.toLocaleString('ru-RU', { minimumFractionDigits: 2 })}</span>
