...
// Мок-данные для теста (убрать при интеграции с бэком)
...
const TradesList: React.FC<TradesListProps> = ({ trades }) => {
  ...
  // Первая и последняя точки — низ (y=height)
  ...
  return (
    <Card className="relative w-full min-w-[340px] max-w-xl mx-auto p-0 bg-light-card dark:bg-dark-card border border-light-border/60 dark:border-dark-border/60 rounded-2xl shadow-sm flex flex-col gap-2">
      ...
      <div className="relative z-10">
        {/* SVG фон */}
        <svg
          className="absolute inset-0 w-full h-full z-0 pointer-events-none select-none text-light-accent/30 dark:text-dark-accent/30"
          width={width}
          height={height}
        >
          <defs>
          ...
          <path d={path} fill="none" stroke="currentColor" strokeWidth="2" opacity="0.55" />
        </svg>
        {/* Контент поверх */}
        <div className="relative z-10 flex flex-col">
          ...
          <div
            className="divide-y divide-light-border/20 dark:divide-dark-border/20 overflow-y-auto"
