...
const tabStyles = (
  ...
  `flex-1 text-center py-2 text-sm font-bold transition-colors duration-150 cursor-pointer
   ...
   border-b-2 border-b-transparent ${
     active
       ? buy
         ? "border-b-light-success dark:border-b-dark-accent"
         : "border-b-[#e0a6a6] dark:border-b-[#6d2323]"
       : ""
   }
   -mr-px`;
...
const TradeFormWithTabs: React.FC<TradeFormWithTabsProps> = ({
  sideDefault = "buy",
}) => {
  ...
  // --- portfolio state ---
  const [portfolioInstruments, setPortfolioInstruments] = useState<
    { instrumentId: number; availableAmount: number; ticker: string }[]
  >([]);
  ...
  useEffect(() => {
    if ((instruments as InstrumentWithPrice[]).length && instrument) {
      const found = (instruments as InstrumentWithPrice[]).find(
        (inst) => inst.ticker === instrument
      );
  ...
  useEffect(() => {
    if ((instruments as InstrumentWithPrice[]).length && !instrument) {
      ...
      if ((instruments as InstrumentWithPrice[])[0].price)
        setPriceRub((instruments as InstrumentWithPrice[])[0].price!);
    }
  }, [instruments, instrument]);
  ...
  const selectedInstrument = useMemo(
    () => instruments.find((inst) => inst.ticker === instrument),
    [instruments, instrument]
  );
  const portfolioInstrument = useMemo(
    () =>
      portfolioInstruments.find(
        (inst) => inst.instrumentId === selectedInstrument?.instrumentId
      ),
    [portfolioInstruments, selectedInstrument]
  );
  ...
  const isAmountValid =
    !!amount && Number(amount) > 0 && (tab === "buy" ? canBuy : canSell);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!amount || isNaN(Number(amount)) || Number(amount) <= 0) {
      setToast({
        open: true,
        message: "Укажите корректное количество (> 0)",
        type: "error",
      });
      return;
    }
    if (!priceRub || isNaN(Number(priceRub)) || Number(priceRub) <= 0) {
      setToast({
        open: true,
        message: "Укажите корректную цену (> 0)",
        type: "error",
      });
      return;
    }
    ...
    if (tab === "sell" && Number(amount) > availableToSell) {
      setToast({
        open: true,
        message: "Недостаточно инструмента для продажи",
        type: "error",
      });
      return;
    }
    ...
    if (!portfolioId) {
      setToast({
        open: true,
        message: "Портфель не загружен, попробуйте позже",
        type: "error",
      });
      return;
    }
    if (portfolioId === 1) {
      setToast({
        open: true,
        message:
          "Внимание: portfolioId=1. Проверьте, что вы авторизованы и backend возвращает корректный id.",
        type: "error",
      });
    ...
    try {
      ...
      if (!res.ok) {
        let errorText = "";
        try {
          errorText = (await res.json()).message;
        } catch {
      ...
      setToast({
        open: true,
        message: err.message || "Ошибка создания заявки",
        type: "error",
      });
    }
  }
    ...
    <>
      <div className="w-full min-w-[220px] max-w-[280px] h-full flex flex-col justify-between bg-light-card dark:bg-dark-card border border-light-border/40 dark:border-dark-border/40 rounded-2xl self-start">
        ...
        <div className="flex border-b border-light-border/40 dark:border-dark-border/40">
          <button
            className={tabStyles(tab === "buy", true, true, false)}
          ...
          <button
            className={tabStyles(tab === "sell", false, false, true)}
            onClick={() => setTab("sell")}
          >
            Продать
          </button>
        ...
        <div className="flex gap-1 px-2 pt-1 pb-0.5">
          <button
            className={orderTabStyles(orderType === "limit")}
            onClick={() => setOrderType("limit")}
          >
            Лимит
          </button>
          <button
            className={orderTabStyles(orderType === "market")}
        ...
        {/* Форма */}
        <form
          className="flex flex-col justify-between gap-1 px-2 pb-2 pt-0.5 h-full"
          onSubmit={handleSubmit}
        >
          {/* Инструмент */}
          <label className="text-xs font-semibold text-light-fg-secondary dark:text-dark-brown mb-0.5">
            Инструмент
          </label>
          {loading ? (
            <div className="py-2 text-center text-light-fg-secondary dark:text-dark-brown">
          ...
          <label className="text-xs font-semibold text-light-fg-secondary dark:text-dark-brown mb-0.5 mt-1">
            Цена
          </label>
          ...
          <div className="flex justify-between items-center mt-0.5">
            ...
            <span className="font-bold text-sm text-light-fg dark:text-dark-fg">
              {totalRub
          ...
          <div className="flex justify-between items-center mt-0.5">
            ...
            <span className="font-semibold text-light-fg dark:text-dark-fg text-xs">
              {balance} RUB
            </span>
          </div>
          <div className="flex justify-between items-center mt-0.5">
          ...
          <button
            ...
            ${
              tab === "buy"
                ? "bg-light-success/90 text-white dark:bg-dark-accent/90 dark:text-dark-bg"
                : "bg-[#e0a6a6] text-[#7a3a3a] dark:bg-[#6d2323] dark:text-[#e0a6a6]"
            }
