...
function getErrorMessage(err: string) {
  if (/400/.test(err) && /position|values|contains/i.test(err))
    return "Пользователь содержит позиции по инструменту!";
  ...
  const total = useMemo(
    () =>
      instrumentsWithValue.reduce(
        (sum, a) =>
          sum + (a.totalPrice ?? (a.price || 1) * (a.totalAmount || 0)),
        0
      ),
  ...
  const pie = useMemo(
    () =>
      instrumentsWithValue.map(
        (a) => (a.totalPrice ?? a.totalAmount * (a.price || 1)) / (total || 1)
      ),
    [instrumentsWithValue, total]
  );
            ...
            >
              <div className="flex items-center gap-4 mb-1">
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 px-2 py-0.5 rounded bg-light-accent/10 dark:bg-dark-accent/10 text-light-accent dark:text-dark-accent text-xs font-semibold">
                      {a.type}
                    </span>
                  </div>
                ...
                <span className="text-[20px] font-bold text-green-600 dark:text-green-400 ml-auto whitespace-nowrap">
                  ₽{" "}
                  {formatNumber(
                    a.totalPrice ?? (a.price || 1) * (a.totalAmount || 0),
                    2
                  )}
                </span>
              </div>
              <div className="flex flex-row flex-wrap gap-6 items-center text-[15px] font-medium">
                <div className="flex flex-col">
                  <span className="text-xs text-light-fg/60 dark:text-dark-brown/70">
                    Доступно
                  </span>
                  <span className="font-mono text-[16px] font-bold text-light-fg dark:text-dark-fg">
                    {formatNumber(a.availableAmount)}
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-light-fg/60 dark:text-dark-brown/70">
                  ...
                  <span className="font-mono text-[16px] text-light-fg/70 dark:text-gray-500">
                    {a.blockedAmount ? formatNumber(a.blockedAmount) : "—"}
                  </span>
                </div>
                <div className="flex flex-col">
                  <span className="text-xs text-light-fg/60 dark:text-dark-brown/70">
                ...
                <div className="flex flex-col ml-auto">
                  ...
                  <span className="font-mono text-[16px] font-bold text-light-accent dark:text-dark-accent">
                    ₽{" "}
                    {formatNumber(
                      a.totalPrice ?? (a.price || 1) * (a.totalAmount || 0),
                      2
